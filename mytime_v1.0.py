...
def export_ics(filepath: str, events_rows):
    ...
    lines = ["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Structured-Python//EN"]
    for r in events_rows:
        dtstart = datetime.fromisoformat(r["start_dt"]) ; dtend = datetime.fromisoformat(r["end_dt"]) 
        lines += ["BEGIN:VEVENT", f"SUMMARY:{r['title']}", f"DTSTART:{fmt(dtstart)}", f"DTEND:{fmt(dtend)}", f"UID:{r['id']}@structured-python", "END:VEVENT"]
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Structured-Style Planner (Python)")
